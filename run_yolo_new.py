...
if __name__ == "__main__":
    ...
    cap = cv2.VideoCapture("/home/ml/code/python/video/traffic.mp4")
    while True:
        ...
        limX1, limY1 = (im_w/4)+(im_w/4)/2, (im_h/4)+(im_h/4)/2 #(480, 270)
        limX2, limY2 = (im_w/2)+(im_w/4)/2, (im_h/2)+(im_h/4)/2 #(800, 450)
        ...
        for box in boxes:            
            ...
            #Calcula o centro da imagem identificada dentro do box
            cv2.rectangle(im_out, ((xl+xr)/2-1, (yt+yb)/2-1), ((xl+xr)/2+1, (yt+yb)/2+1), (0,0,255), 3)

            # print "CLASSES[box.get_label()]: ", CLASSES[box.get_label()]
            if (CLASSES[box.get_label()] == "person") and (xr >= limX1 and xr <= limX2) and (yb >= limY1 and yb <= limY2):
                cv2.putText(im_out, ">>STOP<<", (320,450), cv2.FONT_HERSHEY_SIMPLEX, 4, (0,255,255), 3)
